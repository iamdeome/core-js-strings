...
function isPalindrome(str) {
  const tempString = str.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
  let s = 0;
  let e = tempString.length - 1;
  while (s < e) {
    if (tempString[s] !== tempString[e]) {
      return false;
    }
    s += 1;
    e -= 1;
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (letter === letter.toUpperCase()) {
      invert += letter.toLowerCase();
    } else {
      invert += letter.toUpperCase();
    }
  }
  return invert;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (char >= 65 && char <= 77) {
      res += String.fromCharCode(char + 13);
    } else if (char > 77 && char <= 90) {
      res += String.fromCharCode(char - 13);
    } else if (char >= 97 && char <= 109) {
      res += String.fromCharCode(char + 13);
    } else if (char > 109 && char <= 122) {
      res += String.fromCharCode(char - 13);
    } else {
      res += str[i];
    }
